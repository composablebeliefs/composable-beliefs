---
name: position
description: Capture a settled stance as an anchored position document in the host repo's positions/ - verbatim passages, typed claims, resolver-verified anchors. Applies the ceremony gate first (cb:a496); extraction into the graph stays with /assert. Never writes beliefs/.
---
...
2. **Write the header:** `# Position: <title>`, then `**Type:** position`,
   `**Class:** <e.g. design-policy>` (cb:a511), `**Status:** active`,
   `**Authored:** <date>`, `**Origin:** <thread description,
   session:<date>-<slug>>`. A `session:` slug is a dead end for a fresh reader
   (cb:a507, cb:a518) - the document itself must carry everything needed to
...
4. **Anchor claims about code.** Each such claim carries one or more
   ...
   claim terminates in the substrate (cb:a485); a claim about code with no
   resolvable anchor is ungrounded (cb:a488).
5. **Verify every anchor through the draft-mode gate (cb:a512).** Write the
   ...
   (cb:a488). Treat a loose-anchor warning as a failure at authoring time:
...
8. **After import, flip the records:** for each extracted claim, change
   ...
   rendered per the cb:a503 convention - an italic gloss (enough meaning to
   ...
   While a503 is still settling through use, gloss + id is the floor; add the
...
- **Capture at settle time.** The worked trace's failure mode: rationale
  ...
  slug and was nearly lost (cb:a518). Appending later is recovery, not the
...
- **Don't lower the gate.** If every wart became a three-artifact pipeline,
  the pipeline would stop being read (cb:a496).
