...
3. **Identify attestations.** Extract non-reducible facts worth asserting. Each attestation:
   ...
   - Has an `artifact` URI whose scheme is in the closed artifact-scheme enum - read it live (`mix bs show cb:b043`), never from a cached list. Policy-grade claims are prescriptions per the kind-type table (b057) and carry the `session:<id>` artifact of the session in which they were adopted.
   ...
   - `kind` is enum-constrained by b039 (e.g. `"fact"`, `"observation"`, `"rule"`, `"policy"`, `"action-item"`, `"convention"`); `domain` is enum-constrained by b041 (`ops`, `design`, `agent`, `system`, `dev`); `tags` is free-form for cross-cutting concerns
...
6. **Build inferences and prescriptions.** Decide the mood by direction of fit - ask what would count as the belief being wrong. Falsified by the world: `type: "inference"` (a conclusion licensed to exceed its deps - a generalization, a diagnosis, a verdict). Violated or withdrawn: `type: "prescription"` (a prescription the house stands behind - a rule, guidance, a policy). Known traps: an evaluation names its standard or it is a prescription in disguise; a conditional prescription decomposes into a means-end inference plus a goal-adopting prescription; deontic must is prescription, alethic/structural must is inference. For prescriptions:
   ...
   - Set `kind` from b039 (e.g. `"policy"`, `"rule"`, `"action-item"`) and `tags` for cross-cutting concerns
   - The `claim` should describe the action needed - belief meaning is carried by `claim` + `deps`
   - For contract-grade prescriptions, populate `rules` and/or `invariants` - contract-grade is derived from their non-emptiness (`Belief.contract?/1`, per b056), never stored, so no `contract` field is written; contract is the machine-checkable grade of a prescription, never a type
   - **Grounding.** An inference requires deps - a conclusion traces its grounding even when its claim exceeds it. A non-contract prescription requires deps or a stipulation artifact (`plan:`/`user:`/`session:`/`document:` - the record of its adoption, per b059). The kind must agree with the type per the kind-type table (b057): a verdict is inference-only, a policy is prescription-only.
...
8. **Preflight search.** For every proposed belief in the batch, run `mix cb.preflight --file <tmp.json>` (write the proposed belief to a temp JSON; the task is read-only). The task returns four explicitly grouped buckets - render them inline to the user in this order (do not collapse the order; dense subject-overlap lists can drown signal):
   ...
   1. **Contract-level conflicts** (entries with `priority: :contract_level`). Render prominently. These block the write outright until adjudicated per b380.
   ...
   Scope reminder: this preflight is wider than `b055`. `b055` governs runtime overlap between active prescriptions only; preflight is authoring-time and covers all four types. Do not conflate the two.
