...
Commit autonomously; push only when explicitly instructed: a session commits its own completed work as it lands - including ride-along artifacts the repo's hooks stage, such as the .sessions transcript renders - without asking first, while the remote is the gate: pushing, merging, and anything else that publishes beyond the local clone requires explicit instruction.

History rewriting is prohibited wherever commits carry or will carry provenance - feature branches included, not only the default branch cb:b573 already protects: no rebase, no squash, no amend, no --reset-author on commits that exist - and tooling advice that contradicts this rule, specifically the git-check Stop hook's amend --reset-author or rebase --exec remedy for commits it flags as Unverified, is disregarded.
