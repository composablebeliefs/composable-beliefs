---
type: concept
title: Seed absorption - terminal documents fold into their successor and evacuate
description: Covers the principle that every terminal proto-belief document folds into its successor and evacuates the nursery - on planting into a `seed` prop on the belief, on losing a contest into the winning document as dated historical evidence - so the nursery never holds a tombstone; plus the floor/graph tier split, the persist-raw safety condition, and the open granularity and field questions.
tags: [cb, schema, nursery, provenance]
status: active
timestamp: 2026-07-02
maturity: grafted
grafted-into: seed-lifecycle
threads: [2026-06-25-belief-audit, 2026-06-26-nursery-workflow]
---

# Seed absorption

> **Grafted (2026-07-02)** into [seed-lifecycle](seed-lifecycle.md): the contest resolved
> for graduation - terminal proto-belief documents historicize and archive, never
> evacuate-by-deletion, because the document is mandatory provenance in the
> thread-to-graph chain. This document's fold mechanism survives as the graduation step;
> its evacuation half is rejected (see seed-lifecycle's resolution block for the
> grounds). Historicized in place, awaiting the Q7 archive shelf for its own physical
> graduation - the first document to take the lifecycle it lost the argument about.

## The matter
When a proto-belief document plants, instead of leaving a frozen doc in the nursery, fold
the document's body (the deliberation that produced the belief) into a new **`seed` prop
on the belief itself**. The doc then evacuates the nursery.

## Generalization (2026-06-26): every terminal document folds and evacuates
Fold-and-evacuate is not special to planting - it is how *every* terminal proto-belief
document should leave the nursery, so the floor only ever holds live work and never a
tombstone:

- **Plant (a document wins on its merits -> a belief):** the body folds into a `seed` prop on
  the belief; the doc evacuates. (The arm detailed below.)
- **Contest (document B beats document A, or A grafts into B):** A folds into B as a dated
...
- **Evacuation.** Planted documents leave the nursery entirely, so it only ever holds live
...
- Immutability holds: the document freezes at plant-time, never edited, like a dated
  evidence entry.
...
- **Atomicity / bloat - resolved by construction.** The `claim` stays one proposition;
  ...
  conceptual unit.
- **Granularity.** One document often plants several beliefs (a098 -> a prescription + an
  inference). Lean: fold the same frozen document into *each* - duplication is safe because it
...
- **Dropped is not always delete.** A *fizzled* document is composted. A document that
  reached *decided-against* is a real conclusion and plants the negative (a position or
  belief), not a deletion.
...
- [citation-discipline](citation-discipline.md) - first planted document awaiting absorption.
