...
description: Covers linking each user/agent statement in a thread to the proto-belief document or belief it is synthesized into - the back-edge of transcript-format's excerpts rule (document to exchange), making transcript-to-graph traceability bidirectional at statement granularity.
...
## The matter
Each statement (user or agent) in a thread should carry a way to reach the artifact -
proto-belief document or belief - it was synthesized into. Today the link runs one way: a
proto-belief document quotes its supporting exchanges (transcript-format's excerpts
rule). This is the back-edge: exchange -> document/belief.

## Where it stands
- **Bidirectional traceability at statement granularity.** Forward (document -> exchange) is the
  excerpt; backward (exchange -> document/belief) is this. Together they are the OKF
...
a losing document folds into the winner and is deleted only because its primary reasoning stays
...
## Related
- [transcript-format](transcript-format.md) - the forward edge (documents carry excerpts) and
