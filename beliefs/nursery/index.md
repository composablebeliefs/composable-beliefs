...
- [end-skill-redesign](end-skill-redesign.md) - active - redesign /end so the finalized thread document cannot overstate its completeness: the cb:a518 tail-gap lands on the curated close artifact (the first /end close, fold-chronicle, omits its own close turn while its provenance note claims to include it); the decisive variable is turns not skills (a same-turn /final does not close the gap), and the stronger fix drops the frozen transcript body for a pointer to the continuously-rendered transcript. Render-pointer form gated on transcript-format's repo-weight question. Gates the /end rewrite.
