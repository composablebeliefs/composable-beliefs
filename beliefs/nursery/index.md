...
contested documents are the ones to keep visible, the way `mix bs list tag:lifecycle:discrete`
surfaces the desk.
...
- [statement-provenance](statement-provenance.md) - active - link each thread statement to the artifact it feeds (the back-edge of the excerpts rule).
- [route-tagging](route-tagging.md) - active - per-paragraph, multi-ref route tags placed on the frozen thread body at finalization, keyed on the routing-ledger's routed-to id, so a matter's cross-thread discussion aggregates into an append-only, date-stamped excerpt log in its proto-belief document; the concrete form of statement-provenance's back-edge and a third motion of /route (renamed from /decompose). Unminted; rows staged.
- [council-mechanism](council-mechanism.md) - active - the /council review-and-converge round: a draft PR as the medium (comments, never committed files), an adjudicate-style close-gate, decisions distilled into the target proto-belief and the raw exchange cited by commit; the provenance-tiering rule that keeps a dev-dev review round out of the thread listing while operator design deliberation mints a thread. Unminted; rows staged.
- [per-belief-files](per-belief-files.md) - planted - one JSON file per node; minted cb:a554 + the a555-a560 plan.
- [routing-ledger](routing-ledger.md) - active - per-thread dispatch table (topics, strand states, pointers, dangling questions) so a non-linear thread resumes from its ledger, not memory; a router, never a digest. Minted cb:b572, re-issued as cb:b582 (vocabulary); the /route skill build (renamed from /decompose) stays open.
- [mint-manifest](mint-manifest.md) - active - typed candidate-belief rows inside a maturing proto-belief document, the adopted weak form of the rejected typed-nursery-documents proposal; action items are prescription rows. Minted cb:b567, re-issued as cb:b581 (vocabulary).
...
- [git-check-hook-upstream-fix](git-check-hook-upstream-fix.md) - active - the non-cosmetic upstream fix for the git-check Stop hook's rewrite advice: provision a real in-session signing key (kills the Unverified premise), or drop the amend/rebase remedy for "identity correct, signatures apply at push time", or at minimum gate the remedy on a provenance-repo marker. Lives in the user-global CCR layer no repo can reach; cb:b585 is the in-repo defense, not the fix. Action-item row staged, gated on naming a delivery channel.
- [upstream-request-git-check-hook](upstream-request-git-check-hook.md) - active - the paste-ready upstream request for the git-check Stop hook defect, companion to git-check-hook-upstream-fix; carries the 2026-07-05 revised forensics (commits are SSH-signed at creation and verify on GitHub; the hook's %G? check false-positives without an allowedSignersFile), which supersede the "signing applies at push time" premise. Delivery channel still unnamed; the action-item row stays staged.
