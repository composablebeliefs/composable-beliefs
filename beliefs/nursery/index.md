...
- [end-skill-redesign](end-skill-redesign.md) - active - redesign /end so the finalized thread document cannot overstate its completeness: the cb:a518 tail-gap lands on the curated close artifact (the first /end close, fold-chronicle, omits its own close turn while its provenance note claims to include it); the decisive variable is turns not skills (a same-turn /final does not close the gap). Chosen direction (operator, 2026-07-03): keep the self-contained embedded transcript body, run /end alone in its own later turn so the render has caught up, warn on inline invocation, and keep a completeness-honesty backstop; the render-pointer alternative was considered and rejected on a duplication argument. Gates the /end rewrite.
