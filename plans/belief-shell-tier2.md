...
- On confirmation: assign next ID via existing ID generation pattern, write to DAG via `Store`
...
- Extract the composition-specific logic into a reusable function in a new module: `CB.Belief.Compose`

**Mix task integration:**
- `mix bs compose a050 a051` outputs the proposed compound as formatted detail
...
- Source type must match the source prefix conventions in `belief.ex`

**Existing code to leverage:**
- `/assert` skill handles this entire workflow
- The skill becomes a wrapper that calls `mix bs assert` or the underlying module
...
**Implementation approach:**
- Validate via existing `CB.Belief.Materializer` (must be implication, active, unmaterialized)
...
**Existing code to leverage:**
- `CB.Belief.Materializer` handles the entire write path
...
**New module:** `CB.Belief.Challenge`
...
1. **Skills remain the LLM layer.** `mix bs compose` outputs the deterministic context (assertions, deps, subjects) in a structured format. The agent reads it and does the reasoning. The mix task handles the write on confirmation. The agent is the "probabilistic core" of the kernel.

2. **Add an API call.** The mix task calls an LLM API directly to get a completion. This makes `bs compose` fully self-contained but adds an API dependency.
...
**Recommendation: Option 1 for v1.** The skills already work. The mix task outputs structured context. The agent reasons. The shell's value is in making the boundary explicit, not in hiding the LLM call. Option 2 is the path for the open-source framework but premature for early iterations.

### Mix task routing

Extend `mix bs` with Tier 2 commands:
...
- All 5 operations accessible via `mix bs`
