...
Two addressing conventions: `mix bs <id>` with no verb means `show`, and an id may be written bare (`c051`) or namespaced (`cb:c051`) - a bare id resolves when exactly one belief matches, and returns an ambiguity error rather than guessing when a multi-namespace union has two matches.
...
mix bs show cb:c051                           # 2. inspect one node in full
mix bs tree cb:a489                           # 3. see what a claim rests on
...
**Semantic contact is the escalation bar.** A match escalates into a conflict bucket only when the contract-grade or schema trigger is accompanied by *semantic contact* - a shared subject ref or claim overlap (`cb:c064`). A bare tag-overlap is family resemblance, not contact: two beliefs can both wear `lifecycle:discrete` and have nothing to do with each other. This calibration was itself learned and recorded (`cb:a519`: bare tag-overlap escalated too eagerly), and it enforces at authoring time what the conflict-scope doctrine `cb:c055` states: overlap is necessary but not sufficient for contradiction.
...
The decision becomes a permanent, queryable feature of the graph - a supersession chain, a dependency edge, or a dated deferral - rather than a note in someone's memory. That is the point of the whole arrangement, named by `cb:a387` as the consensus mechanism: conflicts surface at authoring time, resolution routes through recorded adjudication, and contradictions in the governance substrate stay *expensive to introduce* (`cb:a304`), because if contradictions are free, agent behavior drifts silently.
...
- `mix cb.evidence <id> --detail <text> --artifact <uri>` - appends a dated evidence entry, the one in-place growth point on an immutable node (`cb:a522`).
- `mix cb.todo.close <id> --notes <text>` - flips a materialized work item open -> done with discharge notes. Since the `cb:a563` gate, every close must either cite its implementing commit (`--commit <full-sha>`, validated and dereferenced against the repository) or explicitly record that none exists (`--no-commit`); silent omission stops being possible at the door.
- `mix cb.repoint <id> --from <dep> --to <dep>` - swings a dependency from a superseded node to its successor as an atomic drop-then-add, the move that keeps the stale report clean after a supersession (`cb:a537`).
...
**The commit provenance loop.** The `commit:` artifact scheme (`cb:c067`) plus `mix cb.verify.commits` closes the loop between the graph and version control in both directions: every `commit:` URI cited in the graph must dereference to a real commit; every `Belief:` trailer in the repo's commit history must name a live belief; and every todo close recorded with a commit key must dereference too. The belief-to-code link is CI-enforced structure, not prose convention (`cb:a545`, `cb:a563`).

**After the fact: the conflict audit.** The same overlap logic runs as a standing audit, `mix cb.audit.conflicts`, which surfaces stale overrides and scope-overlapping pairs of active prescriptions per `cb:c055`. Read its output as candidates for review, never verdicts: contradictory prescriptions are actionable; contradictory inferences are dissent, out of the audit's scope.

## Obligation lives in the graph

A team's backlog usually lives in a tracker, a plan file, or someone's head. CB puts it in the same graph as everything else (`cb:a489`): work to do lives as prescriptions - grounded, subject-scoped, conflict-audited, staleness-linked, and queryable. The sharp formulation: **a live todo is an unmaterialized discrete prescription.** The backlog is not a second system bolted onto the graph; it is a *view* of the graph, and because the task and the reasoning that justifies it are the same node, the task can never drift away from its own rationale.
...
The materialized field is deliberately modest: it records a historical fact - the action was executed on a date, via a plan - and never asserts the effects still hold (`cb:a383`). Effects decay as code and world drift, which is why `last_verified` is a separate field from `date`, and why drift audits exist as a maintenance cycle (`cb:a384`): re-check materialized beliefs, bump `last_verified` or mint a drift observation.
...
**A plan is what a large prescription materializes into** (`cb:a490`). The files in `plans/` are execution records, never the source of work. Query the graph for *what is next*; follow the prescription into `plans/` for *how and history*. Plans encode intent, not implementation (`cb:a382`, `cb:a375`) - a plan stuffed with sequenced steps becomes a liability the moment the world drifts, because the steps describe a situation that no longer holds while still reading as authoritative.

**Author against the live graph, never from memory** (`cb:a504`). Multiple sessions write the graph concurrently, and a memory snapshot rots the instant another thread pushes. Before minting a backlog node: pull the repos, query the desk, follow the artifacts. The same logic governs resuming: a session resumes from the graph, never from a handoff (`cb:a508`), and session memory is an ephemeral cache, never a store (`cb:a509`). For that to work, a backlog prescription must be **self-bootstrapping** (`cb:a507`): an agent reading it cold can reach the vision, constraints, and prior work through its deps and its resolvable `document:`/`plan:` artifacts. The practice that gets you there is **retro-pairing**: a `session:` stipulation is paired with a `document:` pointer a fresh agent can actually dereference - a directive only its author's memory can interpret is not on the desk in any useful sense.
...
**The digest-file antipattern.** The lesson generalizes past dep edges. The canonical case is recorded as `cb:a386`: a digest file caching the active behavioral prescriptions, regenerated by a command, freshness dependent on a skill telling agents to regenerate after writes. Agents can and do forget; the stale digest is then read as authoritative by the next session. A persisted cache of graph-derived content whose freshness is procedural *embeds the staleness risk it was meant to solve* - the cache becomes a second copy of the truth with no structural tie back to the source. The same shape recurs, and each instance closes the same way:

| The cache | The fix | Belief |
| --- | --- | --- |
| a digest of active prescriptions | load them live: `mix bs list prescription` | `cb:a477` |
| CLAUDE.md as hand-frozen facts | compile it from the graph; CI fails on drift | `cb:a466` |
| the agent's own session memory | treat work state as an ephemeral cache; promote anything load-bearing through the write flow, then prune | `cb:a509` |
...
The unit is the focus, not the session. A conversation touches several focuses and updates whichever documents it concerns; the focus persists and accretes across conversations. This is the atomicity doctrine (`cb:a475`) applied one level up: a document holding several separable focuses is a mis-authored bundle, split at authoring time.
...
> **Grounding.**
> - In the graph: `cb:a489`/`cb:a491` (the desk), `cb:a380`/`cb:a383`/`cb:a384` (status vs materialized; drift audits), `cb:a490`/`cb:a382`/`cb:a375` (plans), `cb:a504`/`cb:a507`/`cb:a508`/`cb:a509` (live-graph authoring, self-bootstrapping, retro-pairing), `cb:a457`/`cb:a533`/`cb:a302` (the hand-edit ban), `cb:c064`/`cb:a519`/`cb:c055` (semantic contact and conflict scope), `cb:a387`/`cb:a304` (the consensus mechanism), `cb:a522`/`cb:a530`/`cb:a537` (the front doors), `cb:c067`/`cb:a545`/`cb:a563` (the commit provenance loop), `cb:a386`/`cb:a477`/`cb:a466`/`cb:a509` (the digest antipattern family), `cb:a501` (recency), `cb:a529` (the anchor gradient), `cb:a475` (atomicity, applied to nursery focuses).
