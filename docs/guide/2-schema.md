...
**Kind binds mood: the kind-type table.** `kind` and `type` are not independent. Contract `cb:b057`, a derivation-table with one row per kind, binds each kind to the structural types it may inhabit:
...
**`claim` is a one-liner.** The provenance fields only work if the claim they support stays small: a single proposition (`cb:b114`). Multi-paragraph analyses are not a belief kind. The right home for a long analysis is a source document that cites belief ids inline, with any durable claim extracted into the graph as its own one-line belief - prose for the narrative, the graph for the atomic claims the narrative rests on.

**Provenance is created at extraction time.** A belief is authored while the agent is actively reading the source, not backfilled later from memory (`cb:b119`) - provenance written from memory is provenance you cannot trust. This rests on a prior distinction: sources are not beliefs (`cb:b113`). The source is raw material; the graph stores the distilled claims that cite it. A graph that swallowed its sources whole would be a copy of everything ever read, and would lose the claim-evidence gap that makes auditing possible.
...
**The rule-kind catalogue.** Contract rules are not free-form prose. They decompose into a closed catalogue (`cb:b046`): each kind pairs a Datalog-shaped declarative fact with exactly one Elixir interpreter. Datalog supplies the fact *shape* only; evaluation lives in ordinary code. The catalogue is closed by its own invariant, so adding a kind supersedes the catalogue contract itself - which is on record as having happened when `output-target` was catalogued.
...
| `cb:b054` | contract-identity | contract-grade iff prescription with non-empty rules/invariants; detect structurally, never by id |
...
> **Grounding.**
> - In the graph: `cb:b482` (the schema lives in `belief.ex` and the graph's contracts, kept in sync by verify.schema), the contract family tabled above, `cb:b397` (enum changes supersede the enum contract), `cb:b398`/`cb:b400` (artifact as typed URI), `cb:b118` (the claim-evidence gap), `cb:b408` (deps versus subjects), `cb:b114` (claim is a one-liner), `cb:b119` (extraction-time discipline), `cb:b113` (sources are not beliefs), `cb:b054`/`cb:b300` (contract identity and the WHAT/HOW/ALWAYS split), `cb:b120` (two roles, one store), `cb:b046`/`cb:b047` (the catalogue and the routing boundary), `cb:b465`/`cb:b417`/`cb:b425` (verification, not generation), `cb:b133`/`cb:b134`/`cb:b135` (neither prose nor code), `cb:b173` (inferences prototype contracts).
