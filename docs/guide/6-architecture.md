...
> **Grounding.**
> - In the graph: `cb:a112` (the single-file rationale) and `cb:a554`-`cb:a560` (the per-belief-file refinement), `cb:a302` (immutability; change adds to the record), `cb:a539` (the deterministic, model-free read path as the protected asset).
