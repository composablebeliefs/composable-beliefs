...
> **Legacy note.** A forward-looking service sketch from before the repo split and the schema epoch: `assertions.json` is now the per-belief `beliefs/cb/` directory, and the `bs compose` / `bs relate` verbs it imagines were never built (see `mix bs help` for the real surface). The runtime rationale stands; read the specifics as vision, not documentation.
...
```
client_app -> reads/writes assertions.json directly
```

**Phase 1 (in-process, same BEAM node):**
```
client_app (GenServer) -> CB.Belief.Store (ETS) -> assertions.json
```
...
This progression is invisible to the agent - beliefs are accessed through the same `CB.Belief.Store` interface at every phase. The infrastructure evolves underneath.
