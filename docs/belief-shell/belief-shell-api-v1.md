...
> **Schema-refresh note (2026-06):** Tier 1 (deterministic ops) shipped as `mix bs`. The `confidence` field has since been removed (replaced by structural support — artifact/evidence/dep counts), so the `low-confidence`/`high-confidence` filters and confidence-colored tree below are obsolete as written; read them as filtering/coloring by structural support. Tier 2/3 (probabilistic/hybrid ops) remain unbuilt — see `../../plans/belief-shell-tier2.md`.
...
bs list subject_type:agent       # assertions about agent subjects
...
- Source: `source:<prefix>` (e.g. `source:user`, `source:document`)
...
```
bs subjects agent/self
bs subjects --type system    # all assertions about system subjects
```
...
bs conflicts subject_type:agent # scan agent-related assertions only
...
**Probabilistic layer:** detect semantic conflicts - claims that contradict each other in meaning even if structurally unrelated. Two facts with no structural link but a real semantic conflict. No one queried for it, but the structure should reveal it.
...
```
bs list primitive source:user | bs relate    # for each user-sourced primitive, find composition candidates
...
|---|---|---|
| `list` | `CB.Belief.Filter` + `Formatter` | exists |
| `show` | `CB.Belief.Formatter.detail/1` | exists |
| `tree` | `CB.Belief.Formatter.tree/2` | exists |
| `deps` | new function on `Filter` or new module | new |
| `dependents` | new - reverse index on `deps` field | new |
| `stale` | `CB.Belief.Filter` + `Formatter.stale_report/2` | exists, extend for cascade |
| `path` | new - BFS on DAG | new |
| `history` | new - walk `superseded_by` chain | new |
| `subjects` | `CB.Belief.Filter` (subject filtering exists) | exists, surface as command |
| `stats` | new - aggregate queries | new |

### Entry point

Single mix task: `mix bs <command> [args]`.
