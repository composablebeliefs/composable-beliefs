> **Historical note.** This is a `patch` — an assertion kind later removed from the schema. Preserved as design history; the underlying RLHF-escalation observations live in the `agent-behavior` belief collection.
...
LAYER 2 - mechanisms discovered through observation (primitives, confidence 0.8-0.9)
  a064: positive feedback escalation (theory -> attributed -> less attributed -> fact)
       [observed in real-time through three revision rounds]
